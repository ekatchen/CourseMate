...
export default function TutorCard({ tutor, highlightCourse }: Props) {
  ...
  return (
    <Link href={`/tutors/${tutor.id}`} className="group block">
      <div className="bg-white rounded-2xl border border-gray-200 p-5 hover:border-brand-700 hover:shadow-md transition-all duration-200 h-full flex flex-col">

        {/* Header row */}
        <div className="flex items-start gap-4">
          <div className="relative w-14 h-14 rounded-2xl overflow-hidden bg-brand-50 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-400 truncate mt-0.5">
              {tutor.program}{tutor.year ? ` · ${tutor.year}` : ""}
            </p>
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
          {displayCourses.map((course) => (
            <span
              key={course.code}
              className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-lg bg-brand-50 text-brand-700"
            >
              <span>{course.code}</span>
              {course.grade && (
                <span className="text-brand-400 font-normal">{course.grade}</span>
              )}
              <span className="text-brand-400 font-normal">${course.ratePerHour}/hr</span>
            </span>
          ))}
          {!highlightCourse && tutor.courses.length > 2 && (
            <span className="inline-flex items-center text-xs font-medium px-2.5 py-1 rounded-lg bg-gray-100 text-gray-400">
