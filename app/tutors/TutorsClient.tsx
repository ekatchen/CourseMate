...
export default function TutorsClient() {
  ...
  const filtered = tutors.filter((t) => {
    ...
    if (!t.sessionFormat) return true;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
      ...
      <form onSubmit={handleSearch} className="flex gap-2 mb-4">
        ...
        />
        <button type="submit" className="text-white bg-brand-700 hover:bg-brand-800 px-4 py-2.5 rounded-lg text-sm font-semibold transition-colors">
          Search
        </button>
        {query && (
          <button type="button" onClick={clearSearch} className="px-4 py-2.5 rounded-lg text-sm text-gray-400 border border-gray-200 bg-white hover:bg-gray-50 transition-colors">
            Clear
          </button>
        )}
      </form>
      ...
      <div className="flex flex-wrap gap-1.5 mb-6">
        {allCourses.map((c) => {
          const active = query.toLowerCase().replace(/\s/g, "") === c.toLowerCase().replace(/\s/g, "");
          return (
            <button
              key={c}
              onClick={() => { setInput(c); setQuery(c); router.push(`/tutors?course=${encodeURIComponent(c)}`); }}
              className={`text-xs px-2.5 py-1 rounded-md font-medium transition-colors ${active ? "bg-brand-700 text-white" : "bg-brand-50 text-brand-700 hover:bg-brand-100"}`}
            >
              {c}
            </button>
          );
      ...
      {loading ? (
        ...
        <div className="py-20 bg-white rounded-xl border border-gray-200 text-center">
          ...
          <p className="text-sm text-gray-400 mb-6">
            {query ? "Leave your details and we'll reach out when one becomes available." : "Try a different format or clear the filter."}
          </p>
          {query && (
            <Link
